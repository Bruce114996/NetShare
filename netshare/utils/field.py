...
class BitField(Field):
    ...
    def normalize(self, decimal_x):
        ...
        #print("decimal_x is ", decimal_x)
        #print("bin_x_0 is ", bin_x)
        bin_x = [int(b) for b in bin_x]
        #print("bin_x_1 is ", bin_x)
        bits = []
        for b in bin_x:
            if b == 0:
                bits += [1.0, 0.0]
...
class Word2VecField(Field):
    ...
    def normalize(self, x, embed_model):
        print("word vec is ", np.array([get_vector(embed_model, str(xi), norm_option=True) for xi in x]))
        return np.array(
