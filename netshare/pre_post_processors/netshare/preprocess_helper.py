...
def df2chunks(big_raw_df,
    ...
    if n_chunks == 1:
        print("time_col_name is ", time_col_name)
...
def apply_per_field(
    ...
    for i, field in enumerate(config_fields):
        ...
        if 'bit' in getattr(field, 'encoding', ''):
            print("current field is ", getattr(field, 'column', ''))
            print("field instance is ", field_instance)
        ...
        if 'categorical' in getattr(field, 'encoding', ''):
            print("field column is ", field.column)
