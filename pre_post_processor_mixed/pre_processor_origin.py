...
class Pre_processor(object):
    ...
    def create_configuration_file(self):
        ...
        for field in ["metadata", "timeseries", "timestamp"]: 
            for col in input_fields[field]:
                format = input_fields[field][col]["format"]
                encoding = input_fields[field][col]["encoding"]
                if "type" in input_fields[field][col]:
                    type = input_fields[field][col]["type"]
                else:
                    type = None 
                if "names" in input_fields[field][col]:
                    names = input_fields[field][col]["names"]
                else:
                    names = None
                if "delimiter" in input_fields[field][col]:
                    delimiter = input_fields[field][col]["delimiter"]
                else:
                    delimiter = None
                res = self.create_json_obj(json_object, field, col, format, encoding, type, names, delimiter)
                if res == False: 
                    col_info = {"name": col, "fields": field}
        ...
        ## format is string: encoding will include { float }
            ...
            if encoding == "list_attributes": 
                for name in names: 
                    ...
                    print("delimiter is ", self.changed_columns[column]["delimiter"])
    ...
    def processor(self):
        ...
        for col in self.deleted_columns:
            for element in data[col['fields']]:
                if element["name"] == col["name"]:
                    data[col['fields']].pop(element)
